...
exports.create = (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
        ...
        blog.save((err, result) => {
              ...
              //res.json(result);
              BlogModel.findByIdAndUpdate(result._id, {
                    ...
                    if (err) {
                          ...
                          BlogModel.findByIdAndUpdate(result._id, {$push: {tags: arrayOfTags}},
                                {new: true}).exec((err, result) => {
                                      if (err) {
                                            return res.status(400).json({
                                                  error: errorHandler(err)
...
exports.remove = (req, res) => {
  const slug = req.params.slug.toLowerCase();
  BlogModel.findOneAndRemove({slug}).exec((err, data) => {
      if (err) {
        return res.json({
          error: errorHandler(err)
        });
...
exports.update = (req, res) => {
    ...
    BlogModel.findOne({ slug }).exec((err, oldBlog) => {
        if (err) {
            return res.status(400).json({
                error: errorHandler(err)
            });
...
exports.listRelated = (req, res) => {
    ...
    BlogModel.find({ _id: { $ne: _id }, categories: { $in: categories } })
        .limit(limit)
        .populate('postedBy', '_id name username profile')
...
exports.listByUser = (req, res) => {
    UserModel.findOne({ username: req.params.username }).exec((err, user) => {
        ...
        let userId = user._id;
        BlogModel.find({ postedBy: userId })
            .populate('categories', '_id name slug')
            .populate('tags', '_id name slug')
            .populate('postedBy', '_id name username')
